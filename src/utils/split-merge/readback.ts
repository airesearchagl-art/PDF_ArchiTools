 ...
 * the orphan pages a copy left outside the page tree, the destination that
 * resolved and navigated nowhere, the JavaScript action detached but never
 * deleted, and the signature widget that rendered pixel for pixel like a
 * signature after the signature was gone.
 */
import { PDFDocument } from 'pdf-lib';
import type { ReadbackFacts } from './contracts';
import { measureDestinationInvariants } from './destinations';
import { scanArtifactWideJavaScript, scanJavaScript } from './javascript';
...
export async function readbackArtifact(bytes: Uint8Array): Promise<ReadbackFacts> {
    ...
    return {
        ...
        artifactWideJavaScript: scanArtifactWideJavaScript(doc),
