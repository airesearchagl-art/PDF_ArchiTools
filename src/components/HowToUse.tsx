...
const releaseHistory: ReleaseNote[] = [
    {
        ...
        version: '1.2.0',
        changes: [
            '注釈を付けたPDFを保存しても、元のPDFのベクターの線や検索できる文字情報が残るようになりました。',
            '書き加えた文字注釈も、PDF上で検索できる文字として保存されます。',
            '文字の書体は、保存時にアプリ同梱の互換フォントへ置き換わります。',
            '互換フォントで表現できない文字を含む文字注釈は、画像として保存され、検索の対象外になります。',
            '表示中のレイヤーだけが保存されます。非表示のレイヤーは出力されません。',
            '消しゴム（ピクセル）を使った部分は、その周囲だけが透明画像として保存されます。',
            '電子署名付き・パスワード保護・破損したPDFは保存できません。理由を表示して中断します。',
            '元のPDFは変更されず、別名（例: drawing_annotated.pdf）で保存されます。',
        ],
    },
    {
        date: '2026/09/07',
        ...
        version: '1.1.2',
        changes: [
            'PDFを開く際、表示処理に必要なPDF.js workerを外部CDNから取得せず、アプリに同梱されたworkerを使用するよう修正しました。',
    ...
    },
    {
        ...
        version: '1.7.0',
        changes: [
            '表題欄の位置を指定して、1ページ1行の図面一覧をExcel（.xlsx）へ書き出せるようになりました。',
            '図面番号・図面名称・版・日付の4項目を、代表ページ上で範囲指定します。',
            '指定した表題欄をどのページに使うかは、ページ番号で明示的に割り当てます。',
            '文字情報がある項目はそのまま読み取り、ない項目だけ画像から認識します。',
            'すべての行を1件ずつ確認するまで、Excelは作成されません。',
            '未割り当てのページも一覧から消えず、理由付きの行として残ります。',
        ],
    },
    {
        date: '2026/09/06',
            ...
            <div className="usage-accordion">
                ...
                >
                    ...
                    <div style={{ ...cardStyle, borderLeft: '6px solid #b0b0b0', marginTop: '20px' }}>
                        ...
                        <ul style={listStyle}>
                            <li>
                                <b>保存されるPDFは、元のPDFに注釈を書き加えたものです。</b>
                                元のPDFが持っていた文字情報・ベクターの線・画像・フォーム・注釈は、
                                保存後のファイルにも残ります。
                                元のPDFは変更されず、別名（例: drawing_annotated.pdf）で保存されます。
                            </li>
                            <li>
                                書き加えた文字注釈は、PDF上で検索できる文字として保存されます。
                                ただし画面で選んだ書体はPDFに埋め込めないため、
                                <b>アプリ同梱の互換フォントへ置き換わります</b>。
                                互換フォントに字形がない文字（絵文字など）を含む文字注釈は画像として保存され、
                                その注釈は検索の対象外になります。保存後に件数を表示します。
                            </li>
                            <li>
                                保存されるのは<b>表示中のレイヤーだけ</b>です。非表示のレイヤーは出力されません。
                            </li>
                            <li>
                                消しゴム（ピクセル）で消した部分は、PDFの図形として表現できないため、
                                その周囲だけが透明画像として保存されます。
                                画像化に必要な大きさが上限（800万ピクセル）を超える場合は、保存を中断します。
                                画質を落としたりページ全体を画像化したりはしません。
                            </li>
                            <li>
                                電子署名付き・パスワード保護・破損したPDF、
                                署名の有無を確認できないPDF、およびXFAフォームを含むPDFは保存できません。
                                強制保存はできません。
                            </li>
                            <li>
                                消しゴムが消すのは書き加えた注釈だけです。
                                <b>元のPDFの内容は消えません（黒塗り・墨消しではありません）。</b>
                            </li>
                ...
                >
                    ...
                    <div style={gridStyle}>
                        ...
                        <div style={{ ...cardStyle, borderLeft: '6px solid #16a085' }}>
                            <h4 style={cardHeadStyle}><FileSpreadsheet size={18} /> 図面一覧（Excel）— 表題欄から1ページ1行の一覧を作る</h4>
                            <p style={noteStyle}>
                                Output Format で「図面一覧（Excel）」を選ぶと、表題欄の位置を指定して
                                <b>1ページ＝1行</b>の図面一覧を作成できます。上の「Excel (.xlsx) 出力」は
                                1ページの中の表を書き出す機能で、こちらは<b>文書全体の図面リスト</b>を作る別の機能です。
                            </p>
                            <ul style={listStyle}>
                                <li>読み取る項目は<b>図面番号・図面名称・版・日付の4項目</b>です。これ以外は取得しません。</li>
                                <li>代表ページの上で、4つの範囲を<b>ご自身でドラッグして指定</b>します。表題欄を自動では探しません。</li>
                                <li>指定した範囲は「プロファイル」として保存し、<b>どのページに使うかをページ番号で割り当て</b>ます。
                                    用紙サイズが同じでも表題欄が違うことがあるため、割り当ては推測しません。</li>
                                <li>用紙サイズが違うページへの当てはめ方は、
                                    <b>「用紙サイズに比例」</b>か<b>「実寸固定」</b>から選びます。図面によってどちらが正しいかは異なります。</li>
                                <li>項目ごとに、<b>文字情報があればそれをそのまま読み取り</b>、無い項目だけ画像から文字認識します。
                                    ページ単位ではなく項目単位で切り替えます。</li>
                                <li>文字認識は<b>この端末の中だけ</b>で行います。ファイルが外部へ送信されることはありません。</li>
                                <li>大きな用紙（A1・A0など）でも、<b>表題欄の範囲だけ</b>を画像化して読み取ります。</li>
                                <li>ページの回転（90/180/270度）にも対応しています。</li>
                                <li>読み取った文字は<b>全文をそのまま確認できます</b>。表示される値は、その全文から取り出した候補です。</li>
                            </ul>
                            <p style={{ ...noteStyle, fontWeight: 600 }}>
                                <b>すべての行を1件ずつ確認するまで、Excelは作成されません。</b>
                                注意表示が付いていない行も確認が必要です。表示される「OCR内部スコア」は
                                認識エンジン自身の指標であり、<b>内容が正しいことの保証ではありません</b>。
                            </p>
                            <ul style={listStyle}>
                                <li><b>未割り当てのページも一覧から消えません。</b>理由付きの行として残ります。
                                    どのプロファイルにあたるかを勝手に推測することはありません。</li>
                                <li>図面番号が同じ行があれば注意表示を出します。文字が完全に一致する場合のみで、
                                    「A-101」と「A101」は別のものとして扱います。</li>
                                <li><b>抜けている図面番号の推定は行いません。</b>図面番号が飛んでいても、
                                    それが欠番なのか意図的なのかは判断できないためです。</li>
                                <li>プロファイルや割り当てを変更すると、確認済みの行は<b>未確認に戻ります</b>。
                                    画面の内容とダウンロードされるファイルが食い違わないようにするためです。</li>
                                <li>プロファイルは<b>その場限り</b>です。別のPDFへの引き継ぎや保存には対応していません。</li>
                                <li>一度に扱えるのは<b>1つのPDF</b>です。複数PDFのまとめ出力には対応していません。</li>
                                <li>すべての値はテキストとして書き出します。「001」のような先頭の0もそのまま保ちます。</li>
                            </ul>
                        </div>
