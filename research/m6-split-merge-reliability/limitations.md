...
- **Real drawings.** The corpus is 109 synthetic documents at the B1 closure
  (38 when this bullet was first written), the largest a 200-page
...
- **Three walked paths with no fixture.** The resource-graph walker follows
  `/AP` state dictionaries, image XObjects below the page, and `/CharProcs`
  streams carrying their own `/Resources`, and none of the three is in a
  fixture. The last was tried once on a throwaway document while the walker was
...
17. **The detector scanned a page's resources and called that the document.**
    ...
    and a group behind a soft mask extracts READY before and after — true when
    this was written, and closed by item 18. Support was not widened.
18. **The soft-mask path was a known blind path, and it stayed one until B1.**
    Item 17 widened the walker to every scope a content stream can open but
    one. A graphics state holds no content stream, so `/ExtGState` was listed
    as not walked — but a soft mask's `/G` is a transparency group form with
    resources of its own. `ocg-extgstate-smask` put a group there and extracted
    **READY**, and the gate recorded that as the measured edge of the walk
    rather than as clean. The Human adoption of M6-H9b made closing it a
    condition of implementation (B1).

    Measured against the walker before the fix, all eight soft-mask fixtures
    extracted READY: two with a group behind the mask (`ocg-extgstate-smask`,
    `smask-shared-g`), three whose mask cannot be read as one
    (`smask-dangling-g`, `smask-malformed`, `smask-g-not-form`), one nested past
    the depth bound (`smask-depth-exceeded`), and the two clean ones. The
    walker now opens `/ExtGState -> /SMask -> /G` and walks `/G` as a form,
