...
| **H5** | export format, and the encoder — **decide before H11** | The memory bound belongs to an **owned** PNG encoder — RGBA8, filter 0, DEFLATE stored blocks, stated chunk sizes — because a formula about `canvas.toBlob` is not a guarantee about it. The price is real and is the decision: stored blocks do not compress, so **4.001 B/px against the browser's 0.031**, roughly 130× larger output. Accept that for a memory guarantee, or keep the browser encoder and narrow the 512 MiB claim to JS-owned buffers with a separate output policy? JPEG has no comparable provable worst case; if it is chosen the allowance must be re-derived. Either way the bytes should be handed back as a `Blob`, not a base64 data URL: 394 MB against 278 MB on an A3 at 300 dpi. |
...
| **H11** | the output sink, and how large a job is accepted | **Decide after H5; the two are one choice.** The phase model bounds a page, not the operation: 5 pages × 4 members is 1044 MB at publish while each page peaks at 157 MB, and 1914 MB as the base64 data URLs `jsPDF.addImage` is handed today. Two paths. **A — memory-resident container, implementation-ready**: an explicit `MAX_OUTPUT_BYTES` of 256 MiB with a fail-closed preflight, which under the owned encoder's 4.001 B/px accepts about **7 A4 pages at 300 dpi with two members** — a drawing set is larger than that. **B — browser-local spool, DEFER**: the lifecycle is measured (bounded write *and* read, nothing published on cancel or supersede, run-scoped namespaces, ownership-aware cleanup, a storage preflight whose third outcome is `unknown`) and the artifact is not — **no comparison PDF was assembled from a spool**, so it is conditional on an **Output Writer Sub-Spike** before production implementation. No external service either way. |
| **H10** | the whole-job **comparison-kernel** work ceiling | **`MAX_COMPARISON_WORK_UNITS = 12,000,000,000` is a recommendation, not a measurement.** One ceiling on the total, not one per page: the export and the change report run over ranges. **Calibrated against `separable-dilation`, the algorithm the planner is bound to** — a unit means about 42× more work under the shipped scan, so the number is meaningless without it. At the worst of five measured figures it projects to about **22 seconds of comparison-kernel time** — `pairChangeMask` on masks that already exist, *not* a user's wait, which would need an end-to-end calibration this research has not done — or **about 173 A4 pages at 300 dpi and 0.5 mm**, which is the form worth having an opinion about. Each further such page is 69,578,880 units, about 0.13 s. If real drawing sets run past ~170 sheets, raise it. Requires approval; never silently degraded to fit. |
