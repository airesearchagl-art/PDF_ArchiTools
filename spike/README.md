...
**Font subsetting is safe here and only here.** `@pdf-lib/fontkit@1.1.1` has a
known CJK subsetting bug. The spike measured it: `subset: true` damages *visible*
Japanese glyphs (1.8% of pixels differ from an unsubsetted embed), but leaves the
cmap and advance widths intact, so text still extracts perfectly. A `Tr 3` layer
never draws an outline, so subsetting is fine for the OCR layer — and cuts the
embedded font from ~1 MB to ~4.6 KB. Do not carry that setting over to text that
is meant to be seen.
